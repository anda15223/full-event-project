  ...
  SELECT jsonb_build_object(
    ...
    'festival_grid', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        ...
        'concepts_count', (SELECT count(*) FROM festival_contracts WHERE festival_id = f.id),
        'operating_entities', (SELECT array_agg(DISTINCT operating_entity) FROM festival_contracts WHERE festival_id = f.id AND operating_entity IS NOT NULL),
        'stalled_count', (SELECT count(*) FROM festival_contracts WHERE festival_id = f.id AND contract_status='stalled')
    ...
    'stats', jsonb_build_object(
      ...
      'contracts_signed', (SELECT count(*) FROM festival_contracts WHERE contract_status='signed'),
      'contracts_total', (SELECT count(*) FROM festival_contracts),
      'active_rules', (SELECT count(*) FROM cross_festival_rules WHERE active=true),
      'total_contacts', (SELECT count(*) FROM festival_contacts),
      'stalled_contracts', (SELECT count(*) FROM festival_contracts WHERE contract_status='stalled')
    )
  ) INTO result;
  RETURN result;
END;
