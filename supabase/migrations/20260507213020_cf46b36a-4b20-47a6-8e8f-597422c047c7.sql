...
SELECT
  fai.festival_id, f.name AS festival_name, f.slug AS festival_slug, f.start_date AS festival_start_date,
  'festival_action_items'::text AS source_table, fai.id AS source_id,
...
SELECT
  ...
  'planned', 'high', NULL, fc.concept_id, c.name,
  CASE
...
SELECT
  ...
  fs.notes, fs.scheduled_start_at::date, fs.scheduled_start_at::time, fs.scheduled_start_at,
...
SELECT
  ...
  ft.notes, ft.pickup_date, NULL::time, ft.pickup_date::timestamptz,
...
-- Source 9 (HOTFIX): outbound legs only
SELECT
  ft.festival_id, f.name, f.slug, f.start_date,
  ...
  tl.status, 'high'::text, NULL::text, NULL::uuid, NULL::text,
  CASE
...
SELECT
  ...
  'open'::text, 'high'::text, fs.name, NULL::uuid, NULL::text,
  CASE
