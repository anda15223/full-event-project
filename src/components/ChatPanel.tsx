  ...
  const send = async (text: string) => {
    ...
    try {
      await streamChat({
        ...
        onError: (err) => {
          updateAssistant(`⚠️ ${err}`);
          setIsLoading(false);
        },
      });
    } catch {
      updateAssistant("⚠️ Failed to connect to AI service.");
      setIsLoading(false);
    }
  };
        ...
        {!isOpen && (
          ...
          >
            <Button
              ...
              className="h-14 w-14 rounded-2xl bg-primary hover:bg-primary/90 text-primary-foreground shadow-lg shadow-primary/25"
            >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-6 right-6 z-50 w-[420px] h-[600px] max-h-[80vh] flex flex-col bg-card border border-border/60 rounded-2xl shadow-2xl shadow-black/10 overflow-hidden"
          >
            {/* Header */}
            ...
            <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
              {messages.length === 0 && (
                <div className="flex flex-col items-center justify-center h-full text-center px-4">
                  ...
                  <h4 className="font-heading font-semibold text-sm mb-1">AI Operations Assistant</h4>
                  ...
                  <div className="grid grid-cols-2 gap-2 w-full">
                    {suggestions.map(s => (
                      <button
                        key={s}
                        onClick={() => send(s)}
                        className="text-left text-xs p-2.5 rounded-xl bg-secondary/60 hover:bg-secondary transition-colors text-foreground/80"
                      >
                        {s}
                      </button>
                    ))}
              ...
              {messages.map((msg, i) => (
                <div key={i} className={`flex gap-2.5 ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                  ...
                  {msg.role === "user" && (
                    <div className="h-7 w-7 rounded-lg bg-foreground/8 flex items-center justify-center shrink-0 mt-0.5">
            ...
            <div className="px-4 py-3 border-t border-border/50 bg-card">
              ...
              >
                <input
                  ...
                  disabled={isLoading}
                  className="flex-1 h-10 px-4 rounded-xl bg-secondary/50 border border-border/50 text-sm placeholder:text-muted-foreground/60 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary/30 transition-all disabled:opacity-50"
                />
                <Button
                  type="submit"
