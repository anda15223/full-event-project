...
import {
  FileText, Copy, Check, CreditCard, Pencil, Mail,
    ...
    >
      <div className="p-5">
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-[10px] text-muted-foreground font-mono">{invoice.confidence ? `${Math.round(invoice.confidence * 100)}% conf` : ""}</span>
        </div>
        ...
        <div className="flex flex-wrap gap-2 mt-4 pt-3 border-t border-border/30">
          ...
          {invoice.status !== "paid" && !confirmPay && (
            <Button
              size="sm"
          ...
          {confirmPay && (
            <div className="flex items-center gap-2">
              <span className="text-xs text-muted-foreground">Mark {invoice.invoice_number} as paid?</span>
              <Button
                size="sm"
                className="rounded-xl text-xs h-7 bg-success hover:bg-success/90 text-success-foreground"
                onClick={() => { markPaid.mutate(invoice); setConfirmPay(false); }}
                disabled={markPaid.isPending}
              >
                Confirm
              </Button>
              <Button size="sm" variant="ghost" className="rounded-xl text-xs h-7" onClick={() => setConfirmPay(false)}>
                Cancel
              </Button>
            </div>
          )}
