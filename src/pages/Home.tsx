...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <nav className="border-b border-border/30 bg-card/70 backdrop-blur-xl sticky top-0 z-50">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="font-heading font-bold text-base tracking-tight">Full Event Project</span>
          </div>
      ...
      <section className="relative overflow-hidden">
        <div className="max-w-6xl mx-auto px-6 py-28 md:py-40">
          <motion.div initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: "easeOut" }} className="max-w-3xl">
            <div className="inline-flex items-center gap-2 mb-6 px-3.5 py-1.5 rounded-full bg-primary/6 border border-primary/10">
            ...
            <h1 className="text-4xl md:text-6xl font-heading font-bold tracking-tight mb-6 leading-[1.08]">
              Your AI-Powered<br />
            ...
            <div className="flex gap-3 flex-wrap">
              <Button onClick={() => navigate("/dashboard")} size="lg" className="bg-primary hover:bg-primary/90 text-primary-foreground shadow-lg shadow-primary/15 h-12 px-7 rounded-xl">
                Enter Dashboard <ArrowRight className="ml-2 h-4 w-4" />
              </Button>
              <Button onClick={() => navigate("/agent/inbox")} size="lg" variant="outline" className="border-border/50 hover:border-primary/25 hover:bg-primary/4 h-12 px-7 rounded-xl">
                View Agents
              </Button>
            </div>
          </motion.div>
      ...
      <section className="max-w-6xl mx-auto px-6 py-24">
        <div className="text-center mb-16">
          <h2 className="text-3xl font-heading font-bold mb-3 tracking-tight">AI Agent Fleet</h2>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
          {agents.map((agent, i) => (
            <motion.div key={agent.title} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08, duration: 0.5, ease: "easeOut" }}
              className="premium-card-hover p-7">
              <div className={`h-11 w-11 rounded-xl ${agent.bg} flex items-center justify-center mb-5`}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-12 text-center">
            {[
              ...
              <div key={feat.label} className="flex flex-col items-center">
                <div className="h-12 w-12 rounded-2xl bg-primary/6 flex items-center justify-center mb-5">
                  <feat.icon className="h-5 w-5 text-primary" />
                </div>
                <h4 className="font-heading font-semibold mb-2">{feat.label}</h4>
      ...
      <footer className="border-t border-border/30 py-10">
        <div className="max-w-6xl mx-auto px-6 text-center text-xs text-muted-foreground">
          Full Event Project — Business Command Center
        </div>
      </footer>
    </div>
  );
